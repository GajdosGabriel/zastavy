    ...
    <div class="pb-2 bg-white rounded-xl shadow-xl z-10 border-2 border-blue-900 mb-4">
        <div class="text-sm md:text-base font-bold p-4 bg-blue-900 text-gray-200 text-center mb-2 rounded-t-xl">
            <router-link :to="{
                ...
                params: { productId: product.id, productSlug: product.slug },
        ...
        <div class="px-2 py-1">
            ...
            <p class="pt-2 md:pt-2 text-xs md:text-sm text-gray-500 text-center">
                <span>
                    ...
                    {{ product.min_order * product.active_price }},- € s DPH
                </span>
            </p>
            ...
            <router-link :to="{ name: 'public.cart.index' }">
                <transition-group enter-active-class="duration-300 ease-out"
                    ...
                    <p v-for="(message, index) in messages" :key="index"
                        class="mb-4 flex items-center justify-between rounded-md border-2 border-green-500 bg-green-300 p-3 shadow-md">
                        <span>Do košíka ste vložili {{ message + " " + product.unit_value }}.</span>
                        <kosikButton class="h-5 w-5 shrink-0" />
                    </p>
                </transition-group>
            </router-link>
        </div>
